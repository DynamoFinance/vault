...
struct Strategy:
    Nonce: public(uint256)
    ProposerAddress: indexed(address)
    Weights: public(uint256)
    TSubmitted: public(uint256)
    TActivated: public(uint256)
    Withdrawn: public(String[256])
    # len(LGov): public(uint256)
    VotesEndorse: public(uint256)
    VotesReject: public(uint256)
...
def removeGuard(GuardAddress: address):
    ...
    Index = position of GuardAddress in self.LGov
    Last_Pos = len(self.LGov) 
    self.LGov[Index] = self.LGov[Last_Pos] 
    self.LGov[Last_Pos] = ZERO_ADDRESS
...
def swapGuard(OldGuardAddress: address, NewGuardAddress: address):
    ...
    Index = position of OldGuardAddress in self.LGov
    self.LGov[Index] = NewGuardAddress
